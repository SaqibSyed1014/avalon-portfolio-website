            ...
            <SectionLayout>
              ...
              <div ref={horizontalRef} className="hidden xl:flex h-[403px] gap-5 will-change-transform">
                {projects.map((project, index) => (
                  <div key={index} className="relative group h-full w-[49%] flex-none hover:scale-[1.02] hover:translate-y-[-10px] transition-all duration-700">
              ...
              <div className="grid md:grid-cols-1 lg:grid-cols-2 xl:hidden gap-2.5">
                {projects.map((project, index) => (
                    <Link key={index} href="/" className="flex h-full flex-col gap-2 rounded-md border border-white/25 p-1.5 md:h-[263px] lg:h-[300px]">
                      <div className="overflow-hidden rounded-[2px] border border-white/15">
                        {/* eslint-disable-next-line @next/next/no-img-element */}
                        <img
                          src={project.image}
